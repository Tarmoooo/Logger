...
void LoggerApp::onConnectRequested(const QString &dllPath,
                                   const QString &plantsPath,
                                   int plantNo)
                                   
{
    emit logLine(QString("Connect requested: dll=%1 plants=%2 plant=%3")
                 .arg(dllPath, plantsPath)
                 .arg(plantNo));

    const std::string dll = dllPath.toStdString();
    const std::string plants = plantsPath.toStdString();

    const bool ok = connectToEmulator(dll.c_str(), plants.c_str(), plantNo);
    emit logLine(ok ? "Connected OK" : "Connect FAILED");
}
